...
class Debris extends THREE.Mesh {
	constructor(pos, color) {
		...
		this.color = color;
		this.geometry = new THREE.SphereGeometry(0.5, 1, 2);
...
(async () => {
	const DebrisArray = await fetchRandomDebris(5)
	// const coords = await fetchRandomDebrisCoords(DebrisArray)

	// var DebrisCoords = Promise.all(DebrisArray.map(async (debris) => {

	// 	let coord = await fetchDebrisData(debris.catalogNumber, new Date());
	// 	console.log(coord)
	// 	return ({ ...debris, ...coord })
	...
	for (let i = 0; i < DebrisArray.length; i++) {
		let coord = await fetchDebrisData(DebrisArray[i].catalogNumber, new Date());
		DebrisCoords.push({ ...DebrisArray[i], ...coord })
	}
	currentState.allDebris = DebrisCoords;
	currentState.allDebris.forEach(coord => {
		myMarble.addOrbital(new Debris([coord.longitude, coord.latitude, coord.height], 0xffff00))
	});
})()
